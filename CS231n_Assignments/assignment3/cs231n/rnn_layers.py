...
def rnn_step_backward(dnext_h, cache):
    ...
    dWh = prev_h.T.dot(dnext_tanh)  # H,H
    dx = dnext_tanh.dot(Wx.T)          # N,D
    dWx = x.T.dot(dnext_tanh)      # D,H
    dprev_h = dnext_tanh.dot(Wh.T)       # N,H
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-2,-1,-1):
      dxl, dprev_hc, dWxc, dWhc, dbc = rnn_step_backward(dh[:,i,:] + dprev_h, cache[i])
      dWx+=dWxc
      dWh+=dWhc
      db+=dbc
      dx[:,i,:] = dxl
...
def word_embedding_backward(dout, cache):
    ...
    # HINT: Look up the function np.add.at  which is samae as    
    #for row in range(N):
    # for col in range(T):
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i, f, o , g = a[:,0:H], a[:,H:2*H], a[:,2*H:3*H], a[:,3*H:4*H]  #[N,H]
    next_c = f * prev_c + i * g
    next_h = o * np.tanh(next_c)
    cache = (x, prev_h, prev_c, Wx, Wh, b, a, np.tanh(next_c) )
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N, H =h0.shape
    N, T, D = x.shape
    cell_state_initial = np.zeros((N, H))
    prev_c = cell_state_initial
    prev_h = h0
    h = np.zeros((N,T,H))
    cache = []
